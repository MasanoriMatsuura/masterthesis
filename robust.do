...
label var hdds "HDDS"
label var preff_shf "share of households adopting diversification within the union"
label var preff_shi "share of households adopting diversification within the union"
save robust.dta, replace
/**** First stage Tobit model
** first stage CRE
xtset a01 year

bysort a01: egen townb=mean(town)
...
** First stage
eststo clear
reghdfe frm_div extension ln_ra ln_rr ln_rs ln_rw ln_rinsd ln_tmpsd ln_ta ln_tr ln_ts ln_tw Male age_hh hh_size schll_hh lvstck lnfrm market road irrigation year2012 year2015 if frm_div<1, vce(r) absorb(a01) res //to remove singleton

eststo: xttobit frm_div extension ln_ra ln_rr ln_rs ln_rw ln_rinsd ln_tmpsd ln_ta ln_tr ln_ts ln_tw Male age_hh hh_size schll_hh lvstck lnfrm market road extension irrigation year2012 year2015 ln_rab ln_rrb ln_rsb ln_rwb ln_rinsdb ln_tmpsdb ln_tab ln_trb ln_tsb ln_twb Maleb age_hhb hh_sizeb schll_hhb lvstckb lnfrmb marketb roadb extensionb irrigationb i.dcode if frm_div<1 & _reghdfe_resid!=., ll(0)  
...
reghdfe inc_div town ln_ra ln_rr ln_rs ln_rw ln_rinsd ln_tmpsd ln_ta ln_tr ln_ts ln_tw  Male age_hh hh_size schll_hh lvstck lnfrm market road irrigation year2012 year2015 , vce(r) absorb(a01) res //to remove singleton

eststo: xttobit inc_div town ln_ra ln_rr ln_rs ln_rw ln_rinsd ln_tmpsd ln_ta ln_tr ln_ts ln_tw   Male age_hh hh_size schll_hh lvstck lnfrm market road irrigation ln_rab ln_rrb ln_rsb ln_rwb ln_rinsdb ln_tmpsdb ln_tab ln_trb ln_tsb ln_twb Maleb age_hhb hh_sizeb schll_hhb lvstckb lnfrmb marketb roadb townb irrigationb i.dcode year2012 year2015 if _reghdfe_resid!=., ll(0) 
quietly estadd local fe Yes, replace //add the raw of fe, year dummy, and control variables
quietly estadd local year Yes, replace 
quietly estadd local control Yes, replace
...
xtgee frm_div extension ln_ra ln_rr ln_rs ln_rw ln_rinsd ln_tmpsd ln_ta ln_tr ln_ts ln_tw Male age_hh hh_size schll_hh lvstck lnfrm market road irrigation year2012 year2015 ln_rab ln_rrb ln_rsb ln_rwb ln_rinsdb ln_tmpsdb ln_tab ln_trb ln_tsb ln_twb Maleb age_hhb hh_sizeb schll_hhb lvstckb lnfrmb marketb roadb extensionb irrigationb if frm_div<1 & _reghdfe_resid!=., family(binomial 100) link(logit) corr(exchangeable) vce(robust)
...
xtgee inc_div town ln_ra ln_rr ln_rs ln_rw ln_rinsd ln_tmpsd ln_ta ln_tr ln_ts ln_tw   Male age_hh hh_size schll_hh lvstck lnfrm market road irrigation ln_rab ln_rrb ln_rsb ln_rwb ln_rinsdb ln_tmpsdb ln_tab ln_trb ln_tsb ln_twb Maleb age_hhb hh_sizeb schll_hhb lvstckb lnfrmb marketb roadb extensionb irrigationb townb year2012 year2015 if _reghdfe_resid!=., family(binomial 100) link(logit) corr(exchangeable) vce(robust) 
predict double xb, mu
...
** First stage
eststo clear
reghdfe frm_div extension ln_ra ln_rr ln_rs ln_rw ln_rinsd ln_tmpsd ln_ta ln_tr ln_ts ln_tw Male age_hh hh_size schll_hh lvstck lnfrm market road irrigation year2012 year2015 if frm_div<1, vce(r) absorb(a01) res 

eststo: reghdfe shnf preff_shf ln_ra ln_rr ln_rs ln_rw ln_rinsd ln_tmpsd ln_ta ln_tr ln_ts ln_tw Male age_hh hh_size schll_hh lvstck lnfrm market road extension irrigation year2012 year2015 if frm_div<1, vce(r) absorb(a01) res //first stage
quietly estadd local fe Yes, replace //add the raw of fe, year dummy, and control variables
quietly estadd local year Yes, replace 
quietly estadd local control Yes, replace

eststo: reghdfe shni preff_shi ln_ra ln_rr ln_rs ln_rw ln_rinsd ln_tmpsd ln_ta ln_tr ln_ts ln_tw  Male age_hh hh_size schll_hh lvstck lnfrm market road irrigation year2012 year2015 if inc_div!=., vce(r) absorb(a01) res
quietly estadd local fe Yes, replace //add the raw of fe, year dummy, and control variables
quietly estadd local year Yes, replace 
quietly estadd local control Yes, replace

esttab using $table\robust_first.tex,  b(%4.3f) se replace wide nogaps starlevels(* 0.1 ** 0.05 *** 0.01) label nocons keep("\textbf{Peer Effect}" preff_shf preff_shi ) order("\textbf{Peer Effect}"  preff_shf preff_shi) s( fe year control N, label("FE" "Year dummy" "Control Variables" "Observations")) 

esttab using $table\robust_first_full.tex,  b(%4.3f) se replace wide nogaps starlevels(* 0.1 ** 0.05 *** 0.01) label nocons s(year control N, label("HH"  "Year dummy" "Control Variables" "Observations"))  order("\textbf{Peer effect}"  preff_shf preff_shi "\textbf{Climate variables}" ln_ra ln_rr ln_rs ln_rw ln_rinsd ln_ta ln_tr ln_ts ln_tw ln_tmpsd "\textbf{Control variables}" Male age_hh hh_size schll_hh lnfrm lvstck irrigation market road extension year2012 year2015) keep("\textbf{Peer effect}" preff_shf preff_shi "\textbf{Climate variables}" ln_ra ln_rr ln_rs ln_rw ln_rinsd ln_ta ln_tr ln_ts ln_tw ln_tmpsd "\textbf{Control variables}" Male age_hh hh_size schll_hh lnfrm lvstck irrigation market road extension year2012 year2015)
...
reghdfe shni preff_shi ln_ra ln_rr ln_rs ln_rw ln_rinsd ln_tmpsd ln_ta ln_tr ln_ts ln_tw  Male age_hh hh_size schll_hh lvstck lnfrm market road irrigation year2012 year2015 if inc_div!=., vce(r) absorb(a01) res //first stage
predict double v2h_fei, r

eststo: xtpoisson hdds shni v2h_fei ln_ra ln_rr ln_rs ln_rw ln_rinsd ln_tmpsd ln_ta ln_tr ln_ts ln_tw   Male age_hh hh_size schll_hh lvstck lnfrm market road irrigation year2012 year2015 if inc_div!=., fe vce(r) //second stage 
quietly estadd local fe Yes, replace //add the raw of fe, year dummy, and control variables
quietly estadd local year Yes, replace 
quietly estadd local control Yes, replace
label var v2h_fei "Residual-income"
...
eststo: reghdfe lnfexp shni v2h_fei ln_ra ln_rr ln_rs ln_rw ln_rinsd ln_tmpsd ln_ta ln_tr ln_ts ln_tw  Male age_hh hh_size schll_hh lvstck lnfrm market road  irrigation year2012 year2015 if inc_div!=., absorb(a01) vce(r) //second stage  idcrp idliv idi_crp_liv, food expenditure
quietly estadd local fe Yes, replace //add the raw of fe, year dummy, and control variables
quietly estadd local year Yes, replace 
quietly estadd local control Yes, replace
...
esttab using $table\robust_alt.tex,  b(%4.3f) se replace nogaps wide starlevels(* 0.1 ** 0.05 *** 0.01) label nocons keep( "\textbf{Diversification}"  frmdiv shnf shni ) order("\textbf{Diversification}"  frmdiv shnf shni ) s(fe year control N, label("FE" "Year dummy" "Control Variables" "Observations")) addnote("Instrumental variables (\% of diversification within unions)")  
...
**quantile inc
reghdfe shni preff_shi ln_ra ln_rr ln_rs ln_rw ln_rinsd ln_tmpsd ln_ta ln_tr ln_ts ln_tw  Male age_hh hh_size schll_hh lvstck lnfrm market road irrigation year2012 year2015 , vce(r) absorb(a01) res //first stage
predict double v2h_fei, r

eststo: xtqreg lnfexp shni v2h_fei ln_ra ln_rr ln_rs ln_rw ln_rinsd ln_tmpsd ln_ta ln_tr ln_ts ln_tw   Male age_hh hh_size schll_hh lvstck lnfrm market road irrigation year2012 year2015, i(a01) quantile(.25)  //second stage  idcrp idliv idi_crp_liv, food expenditure
quietly estadd local fe Yes, replace //add the raw of fe, year dummy, and control variables
quietly estadd local year Yes, replace 
quietly estadd local control Yes, replace
eststo: xtqreg lnfexp shni v2h_fei ln_ra ln_rr ln_rs ln_rw ln_rinsd ln_tmpsd ln_ta ln_tr ln_ts ln_tw   Male age_hh hh_size schll_hh lvstck lnfrm market road irrigation year2012 year2015, i(a01) quantile(  .5 )  //second stage  idcrp idliv idi_crp_liv, food expenditure
quietly estadd local fe Yes, replace //add the raw of fe, year dummy, and control variables
quietly estadd local year Yes, replace 
quietly estadd local control Yes, replace
eststo: xtqreg lnfexp shni v2h_fei ln_ra ln_rr ln_rs ln_rw ln_rinsd ln_tmpsd ln_ta ln_tr ln_ts ln_tw   Male age_hh hh_size schll_hh lvstck lnfrm market road irrigation year2012 year2015, i(a01) quantile(  .75)  //second stage  idcrp idliv idi_crp_liv, food expenditure
quietly estadd local fe Yes, replace //add the raw of fe, year dummy, and control variables
quietly estadd local year Yes, replace 
quietly estadd local control Yes, replace
drop v2h_fei 
